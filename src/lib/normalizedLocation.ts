...
function cleanAddressSeparators(value: string): string {
...
function parseLocationHint(inputAddress: string): Partial<Record<string, string>> {
  const text = cleanString(cleanAddressSeparators(inputAddress));
  if (!text) return {};
...
export function enrichListingInputAttributes(
  ...
  const state = normalizeState(inputAttributes.state) ?? normalizeState(parsed.state);
  ...
  if (city || state || postalCode || country || countryCode) {
    next.locationSource =
