...
def parse_anchor_location_context(pdp_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Try to extract location context from a listing PDP response for nearby search:
    center_lat, center_lng, place_id, query
    """
    out: Dict[str, Any] = {
        "center_lat": None,
        "center_lng": None,
    ...
    # Find place id candidates
    for key in ("placeId", "canonicalPlaceId", "locationPlaceId"):
        vals = _find_keys(pdp_data, key)
        for v in vals:
            if isinstance(v, str) and v.strip():
                out["place_id"] = out["place_id"] or v.strip()
                break

    # Find lat/lng from same object when possible
    for d in _walk_dicts(pdp_data):
        lat = d.get("lat", d.get("latitude", d.get("locationLat")))
        lng = d.get("lng", d.get("longitude", d.get("locationLng")))
        if isinstance(lat, (int, float)) and isinstance(lng, (int, float)):
    ...
    # Query text fallback candidates
    for key in ("city", "localizedCityName", "location", "market", "publicAddress"):
        vals = _find_keys(pdp_data, key)
        for v in vals:
            if isinstance(v, str) and len(v.strip()) >= 3:
                out["query"] = out["query"] or v.strip()
                break
