...
def extract_target_spec(client, listing_url: str) -> Tuple[ListingSpec, List[str]]:
    ...
    if hasattr(client, "get_listing_details"):
        ...
        if listing_id:
            ...
            payload_warnings = [
                "PDP payload extraction skipped: could not parse listing id from URL"
            ]

        try:
            browser_spec, browser_warnings = _extract_target_spec_via_playwright_bridge(client, listing_url)
            if browser_spec is not None:
                return browser_spec, payload_warnings + browser_warnings
            return (
                ListingSpec(url=normalize_airbnb_url(listing_url)),
                payload_warnings + browser_warnings,
            )
        except Exception as exc:
            return (
                ListingSpec(url=normalize_airbnb_url(listing_url)),
                payload_warnings + [f"Browser HTML extraction failed: {exc}"],
            )
